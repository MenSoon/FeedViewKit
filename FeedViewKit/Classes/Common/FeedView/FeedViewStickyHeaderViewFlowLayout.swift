...
class FeedViewFloatingHeaderViewFlowLayout: UICollectionViewFlowLayout {
    ...
    override func layoutAttributesForElements(in rect: CGRect) -> [UICollectionViewLayoutAttributes]? {
        ...
        for itemAttributes in attributes {
            ...
            if itemAttributes.representedElementKind == UICollectionView.elementKindSectionHeader {
                let indexOfSectionObject = visibleSectionsWithoutHeader.firstIndex(of: itemAttributes.indexPath.section)
        ...
        for sectionNumber in visibleSectionsWithoutHeader {
            ...
            if headerSticky {
                if let headerAttributes = self.layoutAttributesForSupplementaryView(ofKind: UICollectionView.elementKindSectionHeader, at: IndexPath.init(item: 0, section: sectionNumber)),
            ...
            if itemAttributes.representedElementKind == UICollectionView.elementKindSectionHeader {
                ...
                if self.headerSticky(section: headerAttributes.indexPath.section) {
                    ...
                    if numberOfSections > headerAttributes.indexPath.section + 1 {
                        if let nextHeaderAttributes = self .layoutAttributesForSupplementaryView(ofKind: UICollectionView.elementKindSectionHeader, at: IndexPath.init(item: 0, section: headerAttributes.indexPath.section + 1)) {
